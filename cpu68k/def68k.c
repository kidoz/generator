              ...
              FILE *outproto)
{
  ...
  *m = '\0';
  ...
  if (*p == '\0') {
    ...
    if (num[bite] || num[bitE] || num[bitf] || num[bitF] || num[biti] ||
        num[bitn] || num[bitN] || num[bitv]) {
    ...
    case 'N':
      ...
      if (!strncmp(p, "Dreg", 4)) {
        stype = dt_Dreg;
        p = p + 4;
      } else if (!strncmp(p, "Areg", 4)) {
        stype = dt_Areg;
        p = p + 4;
      } else if (!strncmp(p, "Aind", 4)) {
        stype = dt_Aind;
        p = p + 4;
      } else if (!strncmp(p, "Ainc", 4)) {
        stype = dt_Ainc;
        p = p + 4;
      } else if (!strncmp(p, "Adec", 4)) {
        stype = dt_Adec;
        p = p + 4;
      } else if (!strncmp(p, "Adis", 4)) {
        stype = dt_Adis;
        p = p + 4;
      } else if (!strncmp(p, "Aidx", 4)) {
        stype = dt_Aidx;
        p = p + 4;
      } else if (!strncmp(p, "AbsW", 4)) {
        stype = dt_AbsW;
        p = p + 4;
      } else if (!strncmp(p, "AbsL", 4)) {
        stype = dt_AbsL;
        p = p + 4;
      } else if (!strncmp(p, "Pdis", 4)) {
        stype = dt_Pdis;
        p = p + 4;
      } else if (!strncmp(p, "Pidx", 4)) {
        stype = dt_Pidx;
        p = p + 4;
      } else {
          ...
          for (cc = 0; (cc == 0) || (num[bitc] && (cc < 16)); cc++) {
            ...
            case i_BSET:
              if (stype == dt_Dreg) { /* dynamic */
                clocks =
                    (dtype == dt_Dreg) ? 8 : 8 + clocks_eacalc(stype, size);
              } else { /* static */
                clocks =
                    (dtype == dt_Dreg) ? 14 : 12 + clocks_eacalc(stype, size);
              }
            case i_BCLR:
              if (stype == dt_Dreg) { /* dynamic */
                clocks =
                    (dtype == dt_Dreg) ? 10 : 8 + clocks_eacalc(stype, size);
              } else { /* static */
                clocks =
                    (dtype == dt_Dreg) ? 14 : 12 + clocks_eacalc(stype, size);
              }
            ...
            case i_BTST:
              if (stype == dt_Dreg) { /* dynamic */
                clocks =
                    (dtype == dt_Dreg) ? 6 : 4 + clocks_eacalc(stype, size);
              } else { /* static */
                clocks =
                    (dtype == dt_Dreg) ? 10 : 8 + clocks_eacalc(stype, size);
              }
            ...
            case i_MOVETSR:
              clocks =
                  (stype == dt_Dreg) ? 12 : 12 + clocks_eacalc(stype, size);
              break;
            case i_MOVETUSP:
            case i_MOVEFUSP:
              clocks = 4;
...
int clocks_eacalc(t_datatype type, t_size size)
{
  ...
  case dt_Areg:
    return 0;
  case dt_Aind:
    return (size != sz_long) ? 4 : 8;
  case dt_Ainc:
    return (size != sz_long) ? 6 : 10; /* should this be 4/8? */
  case dt_Adec:
    return (size != sz_long) ? 6 : 10;
  case dt_Adis:
    return (size != sz_long) ? 8 : 12;
  case dt_Aidx:
    return (size != sz_long) ? 10 : 14;
  case dt_AbsW:
    return (size != sz_long) ? 8 : 12;
  case dt_AbsL:
    return (size != sz_long) ? 12 : 16;
  case dt_Pdis:
    return (size != sz_long) ? 8 : 12;
  case dt_Pidx:
    return (size != sz_long) ? 10 : 14;
...
int clocks_6or8(t_datatype type)
{
  int type_imm = (type == dt_ImmB) || (type == dt_ImmW) || (type == dt_ImmL) ||
                 (type == dt_Imm3) || (type == dt_Imm4) || (type == dt_Imm8) ||
                 (type == dt_Imm8s) || (type == dt_ImmS);
