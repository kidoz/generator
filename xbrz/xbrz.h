...
enum class ColorFormat  // from high bits -> low bits, 8 bit per channel
{
  ...
  ARGB_UNBUFFERED,  // like ARGB, but without the one-time buffer creation
                    // overhead (ca. 100 - 300 ms) at the expense of a slightly
                    // slower scaling time
};
...
DOSBox) then there's no need to run xBRZ on the complete image: Just make sure
you enlarge the source image slice by 2 rows on top and 2 on bottom (this is the
additional range the xBRZ algorithm is using during analysis) CAVEAT: If there
are multiple changed slices, make sure they do not overlap after adding these
additional rows in order to avoid a memory race condition in the target image
data if you are using multiple threads for processing each enlarged slice!
...
avoid processing single rows only; suggestion: process at least 8-16 rows
*/
void scale(
    size_t factor,  // valid range: 2 - SCALE_FACTOR_MAX
    const uint32_t *src, uint32_t *trg, int srcWidth, int srcHeight,
    ColorFormat colFmt, const ScalerCfg &cfg = ScalerCfg(), int yFirst = 0,
...
// parameter tuning
bool equalColorTest(uint32_t col1, uint32_t col2, ColorFormat colFmt,
