...
namespace xbrz {
template <uint32_t N> inline unsigned char getByte(uint32_t val)
{
...
inline uint32_t makePixel(unsigned char a, unsigned char r, unsigned char g,
                          unsigned char b)
{
...
{
  return static_cast<uint16_t>(((pix & 0xF80000) >> 9) |
                               ((pix & 0x00F800) >> 6) |
                               ((pix & 0x0000F8) >> 3));
}
inline uint16_t rgb888to565(uint32_t pix)
{
  return static_cast<uint16_t>(((pix & 0xF80000) >> 8) |
                               ((pix & 0x00FC00) >> 5) |
                               ((pix & 0x0000F8) >> 3));
}
...
template <class Pix> inline Pix *byteAdvance(Pix *ptr, int bytes)
{
  ...
  static_assert(std::is_integral<PixNonConst>::value,
...
template <class Pix>
inline void fillBlock(Pix *trg, int pitch /*[bytes]*/, Pix col, int blockWidth,
                      int blockHeight)
{
...
// nearest-neighbor (going over target image - slow for upscaling, since source
// is read multiple times missing out on cache! Fast for similar image sizes!)
template <class PixSrc, class PixTrg, class PixConverter>
void nearestNeighborScale(const PixSrc *src, int srcWidth, int srcHeight,
                          int srcPitch /*[bytes]*/,
                          /**/ PixTrg *trg, int trgWidth, int trgHeight,
                          int trgPitch /*[bytes]*/, int yFirst, int yLast,
                          PixConverter pixCvrt /*convert PixSrc to PixTrg*/)
{
  static_assert(std::is_integral<PixSrc>::value,
                "PixSrc* is expected to be cast-able to char*");
  static_assert(std::is_integral<PixTrg>::value,
                "PixTrg* is expected to be cast-able to char*");

  static_assert(std::is_same<decltype(pixCvrt(PixSrc())), PixTrg>::value,
...
// nearest-neighbor (going over source image - fast for upscaling, since source
// is read only once
template <class PixSrc, class PixTrg, class PixConverter>
void nearestNeighborScaleOverSource(
    const PixSrc *src, int srcWidth, int srcHeight, int srcPitch /*[bytes]*/,
    ...
    int yFirst, int yLast, PixConverter pixCvrt /*convert PixSrc to PixTrg*/)
{
  ...
  for (int y = yFirst; y < yLast; ++y) {
    ...
    const int yTrgFirst = (y * trgHeight + srcHeight - 1) /
                          srcHeight;  //=ceil(y * trgHeight / srcHeight)
    const int yTrgLast = ((y + 1) * trgHeight + srcHeight - 1) /
                         srcHeight;  //=ceil(((y + 1) * trgHeight) / srcHeight)
...
void bilinearScale(const uint32_t *src, int srcWidth, int srcHeight,
                   ...
                   PixConverter pixCvrt /*convert uint32_t to PixTrg*/)
{
  static_assert(std::is_integral<PixTrg>::value,
                "PixTrg* is expected to be cast-able to char*");
  static_assert(std::is_same<decltype(pixCvrt(uint32_t())), PixTrg>::value,
                "PixConverter returning wrong pixel format");
  ...
  //     -> pre-calculation gives significant boost; std::vector<> memory
  ...
  for (int y = yFirst; y < yLast; ++y) {
    ...
    for (int x = 0; x < trgWidth; ++x) {
      ...
      auto interpolate = [=](int offset) {
        ...
        return c11 * x2xy2y + c21 * xx1y2y + c12 * x2xyy1 + c22 * xx1yy1;
      };
