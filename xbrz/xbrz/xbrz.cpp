...
inline uint32_t gradientRGB(
    uint32_t pixFront,
    uint32_t
        pixBack)  // blend front color with opacity M / N over opaque
                  // background:
                  // https://en.wikipedia.org/wiki/Alpha_compositing#Alpha_blending
{
  static_assert(0 < M && M < N && N <= 1000);

  auto calcColor = [](unsigned char colFront,
                      unsigned char colBack) -> unsigned char {
...
template <unsigned int M, unsigned int N>
inline uint32_t
gradientARGB(uint32_t pixFront,
             uint32_t pixBack)  // find intermediate color between two colors
                                // with alpha channels (=> NO alpha blending!!!)
{
  static_assert(0 < M && M < N && N <= 1000);
...
//{
//     __asm //speeds up xBRZ by about 9% compared to std::sqrt which internally
//     uses the same assembler instructions but adds some "fluff"
//     {
//         fld n
//         fsqrt
//     }
...
struct MatrixRotation {
  static const size_t I_old =
      N - 1 -
      MatrixRotation<static_cast<RotationDegree>(rotDeg - 1), I, J,
...
public:
  OutputMatrix(uint32_t *out, int outWidth)
      :  // access matrix area, top-left at position "out" for image with given
         // width
...
inline double distYCbCr(uint32_t pix1, uint32_t pix2, double lumaWeight)
{
  ...
  // YCbCr conversion is a matrix multiplication => take advantage of linearity
  // by subtracting first!
  const int r_diff =
      static_cast<int>(getRed(pix1)) -
      getRed(
          pix2);  // we may delay division by 255 to after matrix multiplication
  const int g_diff = static_cast<int>(getGreen(pix1)) - getGreen(pix2);  //
  const int b_diff =
      static_cast<int>(getBlue(pix1)) -
      getBlue(
          pix2);  // substraction for int is noticeable faster than for double!
  ...
  const double y =
      k_r * r_diff + k_g * g_diff + k_b * b_diff;  //[!], analog YCbCr!
...
inline double distYCbCrBuffered(uint32_t pix1, uint32_t pix2)
{
  ...
  static const std::vector<float> diffToDist = [] {
    ...
    for (uint32_t i = 0; i < 256 * 256 * 256;
         ++i)  // startup time: 114 ms on Intel Core i5 (four cores)
    {
  ...
  const size_t index = (static_cast<unsigned char>(r_diff / 2)
                        << 16) |  // slightly reduce precision (division by 2)
                                  // to squeeze value into single byte
                       (static_cast<unsigned char>(g_diff / 2) << 8) |
                       (static_cast<unsigned char>(b_diff / 2));

#if 0  // attention: the following calculation creates an asymmetric color
       // distance!!! (e.g. r_diff=46 will be unpacked as 45, but r_diff=-46
       // unpacks to -47
...
FORCE_INLINE  // detect blend direction
    BlendResult
    preProcessCorners(const Kernel_4x4 &ker,
                      const xbrz::ScalerCfg
                          &cfg)  // result: F, G, J, K corners of "GradientType"
{
#if defined _MSC_VER && !defined NDEBUG
  ...
  double jg = dist(ker.i, ker.f) + dist(ker.f, ker.c) + dist(ker.n, ker.k) +
              dist(ker.k, ker.h) + cfg.centerDirectionBias * dist(ker.j, ker.g);
  double fk = dist(ker.e, ker.j) + dist(ker.j, ker.o) + dist(ker.b, ker.g) +
              dist(ker.g, ker.l) + cfg.centerDirectionBias * dist(ker.f, ker.k);

  if (jg < fk)  // test sample: 70% of values max(jg, fk) / min(jg, fk) are
                // between 1.1 and 3.7 with median being 1.8
  {
...
#define DEF_GETTER(x)                            \
  ...
  uint32_t inline get_##x(const Kernel_3x3 &ker) \
  {                                              \
    return ker.x;                                \
  }
// we cannot and NEED NOT write "ker.##x" since ## concatenates preprocessor
// tokens but "." is not a token
DEF_GETTER(a)
DEF_GETTER(b) DEF_GETTER(c) DEF_GETTER(d) DEF_GETTER(e) DEF_GETTER(f)
    DEF_GETTER(g) DEF_GETTER(h) DEF_GETTER(i)
#undef DEF_GETTER

#define DEF_GETTER(x, y)                                             \
  template <> inline uint32_t get_##x<ROT_90>(const Kernel_3x3 &ker) \
  {                                                                  \
    return ker.y;                                                    \
  }
        DEF_GETTER(a, g) DEF_GETTER(b, d) DEF_GETTER(c, a) DEF_GETTER(d, h)
            DEF_GETTER(e, e) DEF_GETTER(f, b) DEF_GETTER(g, i) DEF_GETTER(h, f)
                DEF_GETTER(i, c)
#undef DEF_GETTER

#define DEF_GETTER(x, y)                                              \
  template <> inline uint32_t get_##x<ROT_180>(const Kernel_3x3 &ker) \
  {                                                                   \
    return ker.y;                                                     \
  }
...
#define DEF_GETTER(x, y)                                              \
  template <> inline uint32_t get_##x<ROT_270>(const Kernel_3x3 &ker) \
  {                                                                   \
    return ker.y;                                                     \
  }
                                DEF_GETTER(a, c) DEF_GETTER(b, f)
                                    DEF_GETTER(c, i) DEF_GETTER(d, b)
                                        DEF_GETTER(e, e) DEF_GETTER(f, h)
                                            DEF_GETTER(g, a) DEF_GETTER(h, d)
                                                DEF_GETTER(i, g)
#undef DEF_GETTER


    // compress four blend types into a single byte
    // inline BlendType getTopL   (unsigned char b) { return
    // static_cast<BlendType>(0x3 & b); }
...
inline void clearAddTopL(unsigned char &b, BlendType bt)
{
  b = static_cast<unsigned char>(bt);
}
inline void addTopR(unsigned char &b, BlendType bt)
{
  b |= (bt << 2);
}  // buffer is assumed to be initialized before preprocessing!
inline void addBottomR(unsigned char &b, BlendType bt)
{
  b |= (bt << 4);
}  // e.g. via clearAddTopL()
inline void addBottomL(unsigned char &b, BlendType bt)
{
  b |= (bt << 6);
}  //
...
template <RotationDegree rotDeg>
inline unsigned char rotateBlendInfo(unsigned char b)
{
  return b;
}
...
FORCE_INLINE  // perf: quite worth it!
    void blendPixel(const Kernel_3x3 &ker, uint32_t *target, int trgWidth,
                    unsigned char blendInfo,  // result of preprocessing all
                                              // four corners of pixel "e"
...
#define i get_i<rotDeg>(ker)
  ...
  if (getBottomR(blend) >= BLEND_NORMAL) {
    ...
    const bool doLineBlend = [&]() -> bool {
      ...
      // pixel: handles insular pixels, mario eyes
    ...
    const uint32_t px =
        dist(e, f) <= dist(e, h) ? f : h;  // choose most similar color

    OutputMatrix<Scaler::scale, rotDeg> out(target, trgWidth);

    if (doLineBlend) {
      const double fg =
          dist(f, g);  // test sample: 70% of values max(fg, hc) / min(fg, hc)
                       // are between 1.1 and 3.7 with median being 1.9
      const double hc = dist(h, c);  //

      const bool haveShallowLine =
          cfg.steepDirectionThreshold * fg <= hc && e != g && d != g;
      const bool haveSteepLine =
          cfg.steepDirectionThreshold * hc <= fg && e != c && b != c;
...
public:
  OobReaderTransparent(const uint32_t *src, int srcWidth, int srcHeight, int y)
      : s_m1(0 <= y - 1 && y - 1 < srcHeight ? src + srcWidth * (y - 1)
                                             : nullptr),
        s_0(0 <= y && y < srcHeight ? src + srcWidth * y : nullptr),
        s_p1(0 <= y + 1 && y + 1 < srcHeight ? src + srcWidth * (y + 1)
                                             : nullptr),
        s_p2(0 <= y + 2 && y + 2 < srcHeight ? src + srcWidth * (y + 2)
                                             : nullptr),
        srcWidth_(srcWidth)
  {
  }
...
template <
    class Scaler, class ColorDistance,
    class OobReader>  // scaler policy: see "Scaler2x" reference implementation
void scaleImage(const uint32_t *src, uint32_t *trg, int srcWidth, int srcHeight,
                const xbrz::ScalerCfg &cfg, int yFirst, int yLast)
{
  ...
  //(ab)use space of "sizeof(uint32_t) * srcWidth * Scaler::scale" at the end of
  //the image as temporary buffer for "on the fly preprocessing" without risk of
  // accidental overwriting before accessing
  unsigned char *const preProcBuf =
      reinterpret_cast<unsigned char *>(trg +
                                        yLast * Scaler::scale * trgWidth) -
      srcWidth;

  // initialize preprocessing buffer for first row of current stripe: detect
  // upper left and right corner blending this cannot be optimized for adjacent
  // processing stripes; we must not allow for a memory race condition!
  {
...

    {
      ...
      clearAddTopL(preProcBuf[0],
                   res.blend_k);  // set 1st known corner for (0, yFirst)
    }
    ...
    for (int x = 0; x < srcWidth; ++x) {
      ...
      /*  preprocessing blend result:
          ...
          ---------                                        */
      const BlendResult res = preProcessCorners<ColorDistance>(ker4, cfg);
      addTopR(preProcBuf[x],
              res.blend_j);  // set 2nd known corner for (x, yFirst)

      if (x + 1 < srcWidth)
        clearAddTopL(preProcBuf[x + 1],
                     res.blend_k);  // set 1st known corner for (x + 1, yFirst)
    }
  }
  //------------------------------------------------------------------------------------
  ...
  for (int y = yFirst; y < yLast; ++y) {
    uint32_t *out =
        trg + Scaler::scale * y * trgWidth;  // consider MT "striped" access
    ...
    unsigned char blend_xy1 =
        0;  // corner blending for current (x, y + 1) position
    {
      const BlendResult res = preProcessCorners<ColorDistance>(ker4, cfg);
      clearAddTopL(blend_xy1,
                   res.blend_k);  // set 1st known corner for (0, y + 1) and
                                  // buffer for use on next column

      addBottomL(preProcBuf[0], res.blend_g);  // set 3rd known corner for (0,
                                               // y)
    }
    ...
    for (int x = 0; x < srcWidth; ++x, out += Scaler::scale) {
      ...
      unsigned char blend_xy = preProcBuf[x];  // for current (x, y) position
      {
        ...
        addBottomR(
            blend_xy,
            res.blend_f);  // all four corners of (x, y) have been determined at
                           // this point due to processing sequence!

        addTopR(blend_xy1, res.blend_j);  // set 2nd known corner for (x, y + 1)
        preProcBuf[x] =
            blend_xy1;  // store on current buffer position for use on next row

        [[likely]] if (x + 1 < srcWidth) {
          // blend_xy1 -> blend_x1y1
          clearAddTopL(blend_xy1,
                       res.blend_k);  // set 1st known corner for (x + 1, y + 1)
                                      // and buffer for use on next column

          addBottomL(preProcBuf[x + 1],
                     res.blend_g);  // set 3rd known corner for (x + 1, y)
        }
      }
      ...
      fillBlock(out, trgWidth * sizeof(uint32_t), ker4.f, Scaler::scale,
                Scaler::scale);
      // place *after* preprocessing step, to not overwrite the results while
      // processing the last pixel!

      // blend all four corners of current pixel
      if (blendingNeeded(blend_xy)) {
        const auto &ker3 = reinterpret_cast<const Kernel_3x3 &>(
            ker4);  //"The Things We Do for Perf"
        ...
        blendPixel<Scaler, ColorDistance, ROT_90>(ker3, out, trgWidth, blend_xy,
                                                  cfg);
        blendPixel<Scaler, ColorDistance, ROT_180>(ker3, out, trgWidth,
                                                   blend_xy, cfg);
        blendPixel<Scaler, ColorDistance, ROT_270>(ker3, out, trgWidth,
                                                   blend_xy, cfg);
      }
    }
  }
...
template <class ColorGradient> struct Scaler2x : public ColorGradient {
  ...
  template <unsigned int M,
            unsigned int N>  // bring template function into scope for GCC
  ...
  static void blendCorner(uint32_t col, OutputMatrix &out)
  {
    ...
    alphaGrad<21, 100>(out.template ref<1, 1>(),
...
template <class ColorGradient> struct Scaler3x : public ColorGradient {
  ...
  static void blendLineDiagonal(uint32_t col, OutputMatrix &out)
  {
    alphaGrad<1, 8>(out.template ref<1, 2>(),
                    col);  // conflict with other rotations for this odd scale
  ...
  static void blendCorner(uint32_t col, OutputMatrix &out)
  {
    ...
    // alphaGrad<7, 256>(out.template ref<2, 1>(), col); //0.02826017254 ->
    // negligible + avoid conflicts with other rotations for this odd scale
...
template <class ColorGradient> struct Scaler5x : public ColorGradient {
  ...
  static void blendLineDiagonal(uint32_t col, OutputMatrix &out)
  {
    alphaGrad<1, 8>(out.template ref<scale - 1, scale / 2>(),
                    col);  // conflict with other rotations for this odd scale
  ...
  static void blendCorner(uint32_t col, OutputMatrix &out)
  {
    ...
    // alphaGrad<1, 64>(out.template ref<4, 2>(), col); //0.01676812367 ->
    // negligible + avoid conflicts with other rotations for this odd scale
...
struct ColorDistanceARGB {
  static double dist(uint32_t pix1, uint32_t pix2, double luminanceWeight)
  {
    ...
    /*
    Requirements for a color distance handling alpha channel: with a1, a2 in [0,
    1]

        1. if a1 = a2, distance should be: a1 * distYCbCr()
        2. if a1 = 0,  distance should be: a2 * distYCbCr(black, white) = a2 *
    255
        3. if a1 = 1,  ??? maybe: 255 * (1 - a2) + a2 * distYCbCr()
    */

    // return std::min(a1, a2) * distYCbCrBuffered(pix1, pix2) + 255 * abs(a1 -
    // a2);
    ...
    // alternative? return std::sqrt(a1 * a2 * square(distYCbCrBuffered(pix1,
    // pix2)) + square(255 * (a1 - a2)));
  }
};
...
void xbrz::scale(size_t factor, const uint32_t *src, uint32_t *trg,
                 ...
                 const xbrz::ScalerCfg &cfg, int yFirst, int yLast)
{
  ...
  case ColorFormat::RGB:
    ...
    case 2:
      return scaleImage<Scaler2x<ColorGradientRGB>, ColorDistanceRGB,
                        OobReaderDuplicate>(src, trg, srcWidth, srcHeight, cfg,
                                            yFirst, yLast);
    case 3:
  ...
  case ColorFormat::ARGB:
    ...
    case 2:
      return scaleImage<Scaler2x<ColorGradientARGB>, ColorDistanceARGB,
                        OobReaderTransparent>(src, trg, srcWidth, srcHeight,
                                              cfg, yFirst, yLast);
    case 3:
  ...
  case ColorFormat::ARGB_UNBUFFERED:
    switch (factor) {
    case 2:
      return scaleImage<Scaler2x<ColorGradientARGB>,
                        ColorDistanceUnbufferedARGB, OobReaderTransparent>(
          src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
    case 3:
      return scaleImage<Scaler3x<ColorGradientARGB>,
                        ColorDistanceUnbufferedARGB, OobReaderTransparent>(
          src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
    case 4:
      return scaleImage<Scaler4x<ColorGradientARGB>,
                        ColorDistanceUnbufferedARGB, OobReaderTransparent>(
          src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
    case 5:
      return scaleImage<Scaler5x<ColorGradientARGB>,
                        ColorDistanceUnbufferedARGB, OobReaderTransparent>(
          src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
    case 6:
      return scaleImage<Scaler6x<ColorGradientARGB>,
                        ColorDistanceUnbufferedARGB, OobReaderTransparent>(
          src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
    }
    break;
...
bool xbrz::equalColorTest(uint32_t col1, uint32_t col2, ColorFormat colFmt,
                          double luminanceWeight, double equalColorTolerance)
{
  switch (colFmt) {
  case ColorFormat::RGB:
    return ColorDistanceRGB::dist(col1, col2, luminanceWeight) <
           equalColorTolerance;
  case ColorFormat::ARGB:
    return ColorDistanceARGB::dist(col1, col2, luminanceWeight) <
           equalColorTolerance;
  case ColorFormat::ARGB_UNBUFFERED:
...
{
  bilinearScale(src, srcWidth, srcHeight, srcWidth * sizeof(uint32_t), trg,
                trgWidth, trgHeight, trgWidth * sizeof(uint32_t), 0, trgHeight,
                [](uint32_t pix) { return pix; });
}


void xbrz::nearestNeighborScale(const uint32_t *src, int srcWidth,
                                int srcHeight,
                                /**/ uint32_t *trg, int trgWidth, int trgHeight)
{
  nearestNeighborScale(src, srcWidth, srcHeight, srcWidth * sizeof(uint32_t),
                       trg, trgWidth, trgHeight, trgWidth * sizeof(uint32_t), 0,
                       trgHeight, [](uint32_t pix) { return pix; });
}
