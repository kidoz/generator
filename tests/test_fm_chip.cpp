...
// its public API - YM2612Init / YM2612Write / YM2612UpdateOne - and pins the
// rendered waveform with an FNV-1a hash. It exists because the splash-cart
...
// Save-state plumbing stubs: YM2612_save_state() is not exercised here.
...
void write_reg(uint8_t reg, uint8_t val) {
  REQUIRE(YM2612Write(0, 0, reg) == 0);
  REQUIRE(YM2612Write(0, 1, val) == 0);
}

} // namespace

TEST_CASE("YM2612UpdateOne renders an audible keyed-on carrier", "[fm][chip]")
{
  // Self-contained: clear any chip a previously failed test left behind.
  YM2612Shutdown();
  REQUIRE(YM2612Init(1, FM_CLOCK, FM_RATE, dummy_timer_handler,
                     dummy_irq_handler) == 0);

  write_reg(0x30, 0x06); // ch0 op1 DT=0 MULT=6
  write_reg(0x34, 0x06); // ch0 op2
  write_reg(0x38, 0x06); // ch0 op3
  write_reg(0x3C, 0x06); // ch0 op4
  write_reg(0x40, 0x7F); // op1..op3 total level = silent
  write_reg(0x44, 0x7F);
  write_reg(0x48, 0x7F);
  write_reg(0x4C, 0x10); // op4 (carrier) total level = audible
  write_reg(0x5C, 0x1F); // op4 KS/AR: attack rate = max (0x50 block = KS/AR)
  write_reg(0x6C, 0x07); // op4 AM/DR: decay rate (0x60 block = AM/DR)
  write_reg(0x8C, 0x3F); // op4 sustain level 3, fast release rate
  write_reg(0xA4, 0x22); // block 2, fnum high
  write_reg(0xA0, 0xAA); // fnum low - audible pitch
  write_reg(0xB0, 0x07); // algorithm 7 (all operators parallel)
  write_reg(0xB4, 0xC3); // L+R enable, PMS=3 (LFO phase modulation path)
  write_reg(0x22, 0x38); // LFO enable, high frequency (amd/pmd stepping)
  write_reg(0x28, 0xF0); // key on ch0, all four slots

  std::vector<int16_t> left(OUT_SAMPLES), right(OUT_SAMPLES);
  int16_t *buffers[2] = {left.data(), right.data()};

  YM2612UpdateOne(0, buffers, OUT_SAMPLES);
...
TEST_CASE("YM2612 envelope decays after key-off across update calls", "[fm][chip]")
{
  YM2612Shutdown();
  REQUIRE(YM2612Init(1, FM_CLOCK, FM_RATE, dummy_timer_handler,
                     dummy_irq_handler) == 0);

  write_reg(0x4C, 0x00); // carrier at full level
  write_reg(0x5C, 0x1F); // KS/AR: fast attack
  write_reg(0x6C, 0x00); // AM/DR: slow decay (holds near the attack peak)
  write_reg(0x8C, 0x3F); // SL/RR: fast release once keyed off
  write_reg(0xA4, 0x21);
  write_reg(0xA0, 0x55);
  write_reg(0xB0, 0x07);
  write_reg(0xB4, 0xC0); // L+R, no LFO modulation
  write_reg(0x28, 0xF0); // key on
  ...
  REQUIRE(YM2612Write(0, 0, 0x28) == 0);
  REQUIRE(YM2612Write(0, 1, 0x00) == 0);
