...
void PreferencesDialog::setup_ui() {
    ...
    // Audio Page
    AdwPreferencesPage* audio_page = ADW_PREFERENCES_PAGE(adw_preferences_page_new());
...
void PreferencesDialog::populate_video_page(AdwPreferencesPage* page) {
    ...
    guint selected = 0;
    if (current_scaler) {
        if (g_strcmp0(current_scaler, "scale2x") == 0) selected = 1;
        else if (g_strcmp0(current_scaler, "scale3x") == 0) selected = 2;
        else if (g_strcmp0(current_scaler, "scale4x") == 0) selected = 3;
        else if (g_strcmp0(current_scaler, "xbrz2x") == 0) selected = 4;
        else if (g_strcmp0(current_scaler, "xbrz3x") == 0) selected = 5;
        else if (g_strcmp0(current_scaler, "xbrz4x") == 0) selected = 6;
    }
    adw_combo_row_set_selected(m_scaler_row, selected);

    g_signal_connect_swapped(m_scaler_row, "notify::selected", G_CALLBACK(+[](PreferencesDialog* self) {
        self->on_scaler_changed();
    }), this);
...
void PreferencesDialog::populate_audio_page(AdwPreferencesPage* page) {
    ...
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(m_audio_driver_row), "Audio Backend");

    GtkStringList* c_model = gtk_string_list_new(nullptr);
    gtk_string_list_append(c_model, "Auto-detect");

    const char* current_driver = gtkopts_getvalue("audio_driver");
    if (!current_driver) current_driver = "auto";
    ...
    g_signal_connect_swapped(m_audio_driver_row, "notify::selected", G_CALLBACK(+[](PreferencesDialog* self) {
...
void PreferencesDialog::on_scaler_changed() {
    ...
    const char* val = "none";
    switch (selected) {
        case 1: val = "scale2x"; break;
        case 2: val = "scale3x"; break;
        case 3: val = "scale4x"; break;
        case 4: val = "xbrz2x"; break;
        case 5: val = "xbrz3x"; break;
        case 6: val = "xbrz4x"; break;
    }
