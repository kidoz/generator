...
int asprintf(char **ptr, const char *fmt, /*args */...)
{
  ...
  *ptr = nullptr;
  ...
  str_l = portable_vsnprintf(nullptr, (size_t)0, fmt, ap);
  va_end(ap);
  ...
  *ptr = nullptr;
  {
    ...
    str_l =
        portable_vsnprintf(nullptr, (size_t)0, fmt, ap2); /*get required size */
    va_end(ap2);
  }
  assert(str_l >= 0); /* possible integer overflow if str_m > INT_MAX */
  *ptr = (char *)malloc(str_m = (size_t)str_l + 1);
...
int portable_vsnprintf(char *str, size_t str_m, const char *fmt, va_list ap)
{
  ...
  while (*p) {
    if (*p != '%') {
      ...
      str_arg =
          credits; /* just to make compiler happy (defined but not used) */
      str_arg = nullptr;
      starting_p = p;
      p++; /* skip '%' */
      /* parse flags */
      ...
      if (*p == '.') {
        ...
        if (*p == '*') {
          ...
          if (j >= 0)
             ...
             *   claim that this case should be treated as unspecified
             * precision, which is what we do here.
             */
          }
        } else if (isdigit((int)(*p))) {
          /* size_t could be wider than unsigned int;
      ...
      case 'p': {
         ...
         *   Digital UNIX 4.0, HPUX 10, Linux, FreeBSD, NetBSD; but not with
         * Perl.
         */
      ...
      /* zero padding as requested by the precision or by the minimal field
       * width for numeric conversions required? */
