 ...
 *
 * This implements cycle-accurate timing for the Sega Genesis Video Display
 * Processor (VDP). The Genesis runs at ~7.67 MHz (NTSC) and each scanline takes
 * ~488 CPU cycles.
 *
 ...
 *   State 4: LINE_END      - End of scanline, sync sound and advance to next
 * line
 *
 * The state machine uses fall-through with goto to handle timing precisely:
 * - Each state checks if enough CPU cycles have elapsed (vdp.vdp_nextevent > 0)
...
void event_nextevent(void)
{
  ...
  switch (vdp.vdp_event++) {
  case 0: /* LINE_START: Start of new scanline */
  EVENT_NEWLINE:
    LOG_DEBUG1("%08X due %08X, %d A: %d (cd=%d)", cpu68k_clocks,
                vdp.vdp_event_start, vdp.vdp_line - vdp.vdp_visstartline, vdp.vdp_reg[10],
                vdp.vdp_hskip_countdown);
    /* If first scanline of frame (line 0), initialize sound generation for this
     * field */
    if (vdp.vdp_line == 0)
      sound_startfield();

    /* If we're about to enter the visible display area (line before
     * vdp.vdp_visstartline), clear vertical blank flag and reset H-interrupt
     * counter */
    if (vdp.vdp_line == (vdp.vdp_visstartline - 1)) {
      vdp.vdp_vblank = 0;
      vdp.vdp_hskip_countdown =
          vdp.vdp_reg[10]; /* VDP register 10 = H-interrupt interval */
    }

    /* Calculate cycles until next event (VINT). If not enough time elapsed,
     * break and return */
    if ((vdp.vdp_nextevent = vdp.vdp_event_vint - cpu68k_clocks) > 0)
      break;
  ...
  case 1: /* VINT_CHECK: Vertical interrupt timing */
    LOG_DEBUG1("%08X due %08X, %d B: %d (cd=%d)", cpu68k_clocks,
                vdp.vdp_event_vint, vdp.vdp_line - vdp.vdp_visstartline, vdp.vdp_reg[10],
                vdp.vdp_hskip_countdown);
     ...
     * enabled */
    if (vdp.vdp_line == vdp.vdp_visendline) {
      vdp.vdp_vblank = 1; /* Set vertical blank flag */
      vdp.vdp_vsync = 1;  /* Set vertical sync flag */
      /* VDP register 1 bit 5 = V-INT enable. Trigger 68k interrupt level 6 if
       * enabled */
      if (vdp.vdp_reg[1] & 1 << 5)
        reg68k_external_autovector(
            6); /* Vertical interrupt (highest priority) */
    }

    /* Calculate cycles until next event (HINT). If not enough time, break */
    if ((vdp.vdp_nextevent = vdp.vdp_event_hint - cpu68k_clocks) > 0)
      break;
    vdp.vdp_event++;

  case 2: /* HINT_PROCESS: Horizontal interrupt and DMA processing */
    LOG_DEBUG1("%08X due %08X, %d C: %d (cd=%d)", cpu68k_clocks,
                vdp.vdp_event_hint, vdp.vdp_line - vdp.vdp_visstartline, vdp.vdp_reg[10],
                vdp.vdp_hskip_countdown);

    /* Set horizontal blank flag during active display scanlines */
    if (vdp.vdp_line >= vdp.vdp_visstartline && vdp.vdp_line < vdp.vdp_visendline)
      vdp.vdp_hblank = 1;

    /* Reset H-interrupt counter at boundaries (before visible area or after) */
    if (vdp.vdp_line == (vdp.vdp_visstartline - 1) || (vdp.vdp_line > vdp.vdp_visendline)) {
      vdp.vdp_hskip_countdown = vdp.vdp_reg[10]; /* VDP register 10 = H-INT interval */
      LOG_DEBUG1("H counter reset to %d", vdp.vdp_hskip_countdown);
    }

    /* Horizontal interrupt (H-INT) logic: VDP register 0 bit 4 = H-INT enable
     */
    if (vdp.vdp_reg[0] & 1 << 4) {
      LOG_DEBUG1("pre = %d", vdp.vdp_hskip_countdown);
      /* Decrement H-INT counter. When it reaches 0, trigger interrupt */
      if (vdp.vdp_hskip_countdown-- == 0) {
        LOG_DEBUG1("in = %d", vdp.vdp_hskip_countdown);
        /* Re-initialize counter for next H-INT */
        vdp.vdp_hskip_countdown = vdp.vdp_reg[10];
        LOG_DEBUG1("H counter looped to %d", vdp.vdp_hskip_countdown);

        /* Trigger 68k interrupt level 4 (H-INT) if we're in the right scanline
         * range */
        if (vdp.vdp_line >= vdp.vdp_visstartline - 1 && vdp.vdp_line < vdp.vdp_visendline - 1)
          reg68k_external_autovector(
              4); /* Horizontal interrupt (medium priority) */
        ...
        cpu68k_clocks = vdp.vdp_event_hint;
      }
      LOG_DEBUG1("post = %d", vdp.vdp_hskip_countdown);
    }

    /* DMA (Direct Memory Access) processing: Transfer bytes from 68k RAM/ROM to
     * VRAM The 68k is frozen during DMA (see event_freeze). DMA transfer rate
     * depends on:
    ...
    if (vdp.vdp_dmabytes) {
      ...
      vdp.vdp_dmabytes -=
          (vdp.vdp_vblank || !(vdp.vdp_reg[1] & 1 << 6)) /* Blank or display off? */
              ? ((vdp.vdp_reg[12] & 1) ? 205
                                   : 167) /* Fast transfer (blank period) */
              : ((vdp.vdp_reg[12] & 1) ? 18
                                   : 16); /* Slow transfer (active display) */

      /* DMA complete when counter reaches 0 */
      if (vdp.vdp_dmabytes <= 0) {
        vdp.vdp_dmabytes = 0;
        vdp.vdp_dmabusy = 0; /* Allow 68k to resume */
      }
    }

    /* Calculate cycles until next event (HDISPLAY). If not enough time, break
     */
    if ((vdp.vdp_nextevent = vdp.vdp_event_hdisplay - cpu68k_clocks) > 0)
      break;
    vdp.vdp_event++;

  case 3: /* HDISPLAY: Horizontal display active - trigger UI line rendering */
    LOG_DEBUG1("%08X due %08X, %d D: %d (cd=%d)", cpu68k_clocks,
                vdp.vdp_event_hdisplay, vdp.vdp_line - vdp.vdp_visstartline, vdp.vdp_reg[10],
                vdp.vdp_hskip_countdown);

    /* Notify UI that this scanline is ready to be rendered to the screen.
     * The UI will read vdp_regs[] and vdp.vdp_vram[] to render graphics/sprites */
    generator::ui_line(vdp.vdp_line - vdp.vdp_visstartline + 1);

    /* Calculate cycles until next event (LINE_END). If not enough time, break
     */
    if ((vdp.vdp_nextevent = vdp.vdp_event_end - cpu68k_clocks) > 0)
      break;
    /* Note: vdp.vdp_event++; not needed, we reset vdp.vdp_event to 1 below and jump
     * back */

  case 4: /* LINE_END: End of scanline - sync sound, advance to next line */
    LOG_DEBUG1("%08X due %08X, %d E: %d (cd=%d)", cpu68k_clocks, vdp.vdp_event_end,
                vdp.vdp_line - vdp.vdp_visstartline, vdp.vdp_reg[10], vdp.vdp_hskip_countdown);
    ...
    if (vdp.vdp_vblank || !(vdp.vdp_reg[1] & 1 << 6)) {
      /* Blank period or display off - fast drain */
      vdp_fifo_drain(2);
    } else if ((vdp.vdp_line & 1) == 0) {
    ...
    /* End of frame reached (vdp.vdp_totlines = 262 for NTSC, 313 for PAL) */
    if (vdp.vdp_line == vdp.vdp_totlines) {
      ...
      sound_endfield(); /* Must be first: Finalizes sound buffer for GYM/AVI
                           output */
      generator::ui_end_field(); /* Notify UI that frame is complete */
    ...
    /* Advance all event timers by one scanline (vdp.vdp_clksperline_68k cycles).
     * This shifts the timing window forward for the next scanline. */
    vdp.vdp_event_start += vdp.vdp_clksperline_68k;
    vdp.vdp_event_vint += vdp.vdp_clksperline_68k;
    vdp.vdp_event_hint += vdp.vdp_clksperline_68k;
    vdp.vdp_event_hdisplay += vdp.vdp_clksperline_68k;
    vdp.vdp_event_end += vdp.vdp_clksperline_68k;
...
void event_doframe(void)
{
  ...
  do {
    while (vdp.vdp_nextevent > 0)
      vdp.vdp_nextevent = -reg68k_external_execute(vdp.vdp_nextevent);
    event_nextevent();
...
/* NB: this routine is called by event_freeze which is called by
   ...
   point cpu68k_clocks will have been updated but vdp.vdp_nextevent will not!
   therefore we must at this point work out what vdp.vdp_nextevent should be
...
void event_freeze_clocks(unsigned int clocks)
{
  ...
  /* find out how many clocks vdp.vdp_nextevent has missed */
  /* modify vdp.vdp_nextevent to reflect the real state as of now */

  switch (vdp.vdp_event) {
  case 0:
    missed = vdp.vdp_nextevent - (vdp.vdp_event_start - cpu68k_clocks);
    vdp.vdp_nextevent -= missed;
    break;
  case 1:
    missed = vdp.vdp_nextevent - (vdp.vdp_event_vint - cpu68k_clocks);
    vdp.vdp_nextevent -= missed;
    break;
  case 2:
    missed = vdp.vdp_nextevent - (vdp.vdp_event_hint - cpu68k_clocks);
    vdp.vdp_nextevent -= missed;
    break;
  case 3:
    missed = vdp.vdp_nextevent - (vdp.vdp_event_hdisplay - cpu68k_clocks);
    vdp.vdp_nextevent -= missed;
    break;
  case 4:
    missed = vdp.vdp_nextevent - (vdp.vdp_event_end - cpu68k_clocks);
    vdp.vdp_nextevent -= missed;
    break;
  default:
    printf("assertion failed: bad vdp.vdp_event in event_freeze_clocks: %d\n",
           vdp.vdp_event);
  }

  /* move cpu68k_clocks and vdp.vdp_nextevent forward in time */

  cpu68k_clocks += clocks;
  vdp.vdp_nextevent -= clocks;
  ...
  /* and then un-adjust vdp.vdp_nextevent, as the block marking code will update
     this later */
...
void event_freeze(unsigned int bytes)
{
  ...
  while (togo > 0) {
    ...
    clocks = vdp.vdp_event_end - cpu68k_clocks;
    if (clocks < 0)
      clocks = 0;
    percent_possible = clocks / vdp.vdp_clksperline_68k;
    if (vdp.vdp_reg[1] & 1 << 6 && !vdp.vdp_vblank) {
      /* vdp active */
