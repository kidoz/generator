    ...
    : m_system(std::move(audio), std::move(video), std::move(logger))
{
    ...
    /* The frame driver and sound pipeline reach the backends through the
     * global System. */
    set_system(&m_system);
}
...
void EmulatorCore::reset() { gen_reset(); }

void EmulatorCore::soft_reset() { gen_softreset(); }
...
int EmulatorCore::audio_start() { return soundp_start(); }
void EmulatorCore::audio_stop() { soundp_stop(); }
void EmulatorCore::audio_pause() { soundp_pause(); }
void EmulatorCore::audio_resume() { soundp_resume(); }
int EmulatorCore::audio_samples_buffered() const { return soundp_samplesbuffered(); }
...
int EmulatorCore::video_mode() const { return vdp.vdp_pal; }

unsigned int EmulatorCore::framerate() const { return vdp.vdp_framerate; }
...
const char* EmulatorCore::rom_name() const { return gen_leafname; }

void EmulatorCore::set_debug(int enabled) { gen_debugmode = enabled; }

void EmulatorCore::set_loglevel(int level) { gen_loglevel = level; }

unsigned int EmulatorCore::frame_count() const { return cpu68k_frames; }
