...
/* inline */ WORD Adc_2(WORD wArg1, WORD wArg2)
{
  ...
  m_regF = (((wArg1 ^ q ^ wArg2) & 0x1000) >> 8) | ((q >> 16) & 1) |
           ((q & 0x8000) >> 8) | ((q & 65535) ? 0 : Z_FLAG) |
           (((wArg2 ^ wArg1 ^ 0x8000) & (wArg2 ^ q) & 0x8000) >> 13);
  return q;
#else
  int q = wArg1 + wArg2 + (m_regF & C_FLAG);
  m_regF = (((wArg1 ^ q ^ wArg2) & 0x1000) >> 8) | ((q >> 16) & 1) |
...
/* inline */ WORD Sbc_2(WORD wArg1, WORD wArg2)
{
  ...
  m_regF = (((wArg1 ^ q ^ wArg2) & 0x1000) >> 8) | ((q >> 16) & 1) |
           ((q & 0x8000) >> 8) | ((q & 65535) ? 0 : Z_FLAG) |
           (((wArg1 ^ wArg2) & (wArg1 ^ q) & 0x8000) >> 13) | N_FLAG;
...
#ifdef ARITH_TABLES
  int q = wArg1 + wArg2;
  m_regF = (m_regF & (S_FLAG | Z_FLAG | V_FLAG)) |
           (((wArg1 ^ q ^ wArg2) & 0x1000) >> 8) | ((q >> 16) & 1);
  return q;
#else
  int q = wArg1 + wArg2;
  m_regF = (m_regF & (S_FLAG | Z_FLAG | V_FLAG)) |
...
#else
  int q = m_regA + bArg;
  m_regF = ZSTable[q & 255] | ((q & 256) >> 8) |
           ((m_regA ^ q ^ bArg) & H_FLAG) |
           (((bArg ^ m_regA ^ 0x80) & (bArg ^ q) & 0x80) >> 5);
  m_regA = q;
#endif
}
...
#else
  int q = m_regA - bArg;
  m_regF = ZSTable[q & 255] | ((q & 256) >> 8) | N_FLAG |
           ((m_regA ^ q ^ bArg) & H_FLAG) |
           (((bArg ^ m_regA) & (m_regA ^ q) & 0x80) >> 5);
  m_regA = q;
#endif
}
...
#else
  bArg++;
  m_regF = (m_regF & C_FLAG) | ZSTable[bArg] | ((bArg == 0x80) ? V_FLAG : 0) |
           ((bArg & 0x0F) ? 0 : H_FLAG);
  return bArg;
#endif
}
...
/* inline */ BYTE Dec(BYTE bArg)
{
  ...
  return bArg;
#else
  m_regF = (m_regF & C_FLAG) | N_FLAG | ((bArg == 0x80) ? V_FLAG : 0) |
           ((bArg & 0x0F) ? 0 : H_FLAG);
...
#else
  m_regF = (m_regF & C_FLAG) | H_FLAG |
           ((bArg & (1 << nBit)) ? ((nBit == 7) ? S_FLAG : 0) : Z_FLAG);
#endif
}
...
void InitTables(void)
{
  ...
  for (i = 0; i < 256; ++i) {
    ...
    rgfInc[i] =
        ZSTable[i] | ((i == 0x80) ? V_FLAG : 0) | ((i & 0x0F) ? 0 : H_FLAG);

    //		rgfDec[i] = N_FLAG | ((i == 0x80) ? V_FLAG : 0) | ((i & 0x0F) ? 0 :
    //H_FLAG) | ZSTable[(BYTE)i - 1];

    bArg = i;
    rgfDec[i] =
        N_FLAG | ((bArg == 0x80) ? V_FLAG : 0) | ((bArg & 0x0F) ? 0 : H_FLAG);
    rgfDec[i] |= ZSTable[--bArg];

    for (j = 0; j < 8; j++)
      rgfBit[i][j] =
          H_FLAG | ((i & (1 << j)) ? ((j == 7) ? S_FLAG : 0) : Z_FLAG);
  }
  ...
  for (dw = 0; dw <= 256 * 256; dw++) {
    ...
    nAcc = regA - bArg;
    regF = ZSTable[nAcc & 255] | ((nAcc & 256) >> 8) | N_FLAG |
           ((regA ^ nAcc ^ bArg) & H_FLAG) |
           (((bArg ^ regA) & (regA ^ nAcc) & 0x80) >> 5);
...
unsigned long int mz80exec(unsigned long int cCyclesArg)
{
...
#endif
  {
    ...
    if (cins >= _rgcins[0] && cins <= _rgcins[0] + _rgcins[1] * _rgcins[2] &&
        ((cins - _rgcins[0]) % _rgcins[2]) == 0) {
      if (cins == _rgcins[0]) {
      ...
      TRACE("%08X %04X %04X %04X %04X %04X %04X %04X %04X %04X %s\n", cins,
            GetPC(), m_regAF, m_regBC, m_regDE, m_regHL, m_regIX, m_regIY,
            GetSP(), cCycles, szBuf);
    }
    ...
    case 0xC3:  // jp
...
#else
      {
        ...
        // it isn't correct anyway since "0085 jp 0088" causes is to return
        // (Gyruss) shouldn't it be if (w == wPC - 3)??? jul-26-97 eam
        ...
        if (w == wPc - 6)  // attn: very Galaga specific!!!! should be done in
                           // MageX by patching ROMs with HALTs (once I figure
                           // out how to fool the ROM check at bootup)
        {
          if (*m_rgbOpcode == 0x31) {
...
int HandleED(int cCyclesArg)
{
  ...
  case 0xA1:  // cpi
    cCycles -= 16;
    {
      ...
      ++m_regHL;
      --m_regBC;
      m_regF = (m_regF & C_FLAG) | ZSTable[j] | ((m_regA ^ i ^ j) & H_FLAG) |
               (m_regBC ? V_FLAG : 0) | N_FLAG;
      break;
    }
...
static char *mnemonic_ed[256] = {
    ...
    "!",         "!",         "!",         "!",         "!",
    "!",         "!",         "!",         "!",         "in b,(c)",
    "out (c),b", "sbc hl,bc", "ld (W),bc", "neg",       "retn",
    "im 0",      "ld i,a",    "in c,(c)",  "out (c),c", "adc hl,bc",
    "ld bc,(W)", "!",         "reti",      "!",         "ld r,a",
    "in d,(c)",  "out (c),d", "sbc hl,de", "ld (W),de", "!",
    "!",         "im 1",      "ld a,i",    "in e,(c)",  "out (c),e",
    "adc hl,de", "ld de,(W)", "!",         "!",         "im 2",
    "ld a,r",    "in h,(c)",  "out (c),h", "sbc hl,hl", "ld (W),hl",
    "!",         "!",         "!",         "rrd",       "in l,(c)",
    "out (c),l", "adc hl,hl", "ld hl,(W)", "!",         "!",
    "!",         "rld",       "in 0,(c)",  "out (c),0", "sbc hl,sp",
    "ld (W),sp", "!",         "!",         "!",         "!",
...
static char *mnemonic_xx[256] = {
    ...
    "@",        "add I,de", "@",        "@",        "@",        "@",
    "@",        "@",        "@",        "ld I,W",   "ld (W),I", "inc I",
    "inc Ih",   "dec Ih",   "ld Ih,B",  "@",        "@",        "add I,I",
    "ld I,(W)", "dec I",    "inc Il",   "dec Il",   "ld Il,B",  "@",
    "@",        "@",        "@",        "@",        "inc X",    "dec X",
    "ld X,B",   "@",        "@",        "add I,sp", "@",        "@",
    "@",        "@",        "@",        "@",        "@",        "@",
    ...
    "ld Ih,b",  "ld Ih,c",  "ld Ih,d",  "ld Ih,e",  "ld Ih,h",  "ld Ih,l",
    "ld h,X",   "ld Ih,a",  "ld Il,b",  "ld Il,c",  "ld Il,d",  "ld Il,e",
    "ld Il,h",  "ld Il,l",  "ld l,X",   "ld Il,a",  "ld X,b",   "ld X,c",
    "ld X,d",   "ld X,e",   "ld X,h",   "ld X,l",   "@",        "ld X,a",
    "@",        "@",        "@",        "@",        "ld a,Ih",  "ld a,Il",
    "ld a,X",   "@",        "@",        "@",        "@",        "@",
...
static char *mnemonic_main[256] = {
    "nop",       "ld bc,W",   "ld (bc),a",  "inc bc",     "inc b",
    "dec b",     "ld b,B",    "rlca",       "ex af,af'",  "add hl,bc",
    "ld a,(bc)", "dec bc",    "inc c",      "dec c",      "ld c,B",
    "rrca",      "djnz R",    "ld de,W",    "ld (de),a",  "inc de",
    "inc d",     "dec d",     "ld d,B",     "rla",        "jr R",
    "add hl,de", "ld a,(de)", "dec de",     "inc e",      "dec e",
    "ld e,B",    "rra",       "jr nz,R",    "ld hl,W",    "ld (W),hl",
    "inc hl",    "inc h",     "dec h",      "ld h,B",     "daa",
    "jr z,R",    "add hl,hl", "ld hl,(W)",  "dec hl",     "inc l",
    "dec l",     "ld l,B",    "cpl",        "jr nc,R",    "ld sp,W",
    "ld (W),a",  "inc sp",    "inc (hl)",   "dec (hl)",   "ld (hl),B",
    "scf",       "jr c,R",    "add hl,sp",  "ld a,(W)",   "dec sp",
    "inc a",     "dec a",     "ld a,B",     "ccf",        "ld b,b",
    ...
    "cp (hl)",   "cp a",      "ret nz",     "pop bc",     "jp nz,W",
    "jp W",      "call nz,W", "push bc",    "add a,B",    "rst 00h",
    "ret z",     "ret",       "jp z,W",     "cb",         "call z,W",
    "call W",    "adc a,B",   "rst 08h",    "ret nc",     "pop de",
    "jp nc,W",   "out (B),a", "call nc,W",  "push de",    "sub B",
    "rst 10h",   "ret c",     "exx",        "jp c,W",     "in a,(B)",
    "call c,W",  "dd",        "sbc a,B",    "rst 18h",    "ret po",
    "pop hl",    "jp po,W",   "ex (sp),hl", "call po,W",  "push hl",
    "and B",     "rst 20h",   "ret pe",     "jp (hl)",    "jp pe,W",
    "ex de,hl",  "call pe,W", "ed",         "xor B",      "rst 28h",
    "ret p",     "pop af",    "jp p,W",     "di",         "call p,W",
    "push af",   "or B",      "rst 30h",    "ret m",      "ld sp,hl",
    "jp m,W",    "ei",        "call m,W",   "fd",         "cp B",
    "rst 38h"};
