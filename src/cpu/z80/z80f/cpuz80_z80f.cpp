...
void mirror_to_legacy_context()
{
  ...
  cpuz80_z80.z80afprime = static_cast<std::uint16_t>((r.a_alt << 8) | r.f_alt);
  cpuz80_z80.z80bcprime = static_cast<std::uint16_t>((r.b_alt << 8) | r.c_alt);
  cpuz80_z80.z80deprime = static_cast<std::uint16_t>((r.d_alt << 8) | r.e_alt);
  cpuz80_z80.z80hlprime = static_cast<std::uint16_t>((r.h_alt << 8) | r.l_alt);
...
static unsigned int s_burst_budget =
    0; /* intended Z80 cycles for the current sync burst */
...
void cpuz80_sync(void)
{
   ...
   * into a within-scanline position. See cpuz80_getburstpos(). */
  s_burst_budget = wanted > 0 ? static_cast<unsigned>(wanted) : 0;
...
unsigned int cpuz80_getburstpos(void)
{
   ...
   * Called from YM2612 write handlers while run_for() is executing; the
   * result tells the FM write queue where in the scanline this write
   * retires. Returns 0 outside a burst (degrades to apply-at-line-start). */
  if (s_burst_budget == 0)
    return 0;
  const unsigned long long cycles = s_z80.cycles_since_reset();
  unsigned long long scaled = (cycles * CPUZ80_BURSTPOS_ONE) / s_burst_budget;
