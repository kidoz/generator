...
void CheckKeys() {
	...
	if (key[KEY_1]) INTABLE[0xF7FE] &= 0xFE;
  	if (key[KEY_2]) INTABLE[0xF7FE] &= 0xFD;
	if (key[KEY_3]) INTABLE[0xF7FE] &= 0xFB;
	if (key[KEY_4]) INTABLE[0xF7FE] &= 0xF7;
	if (key[KEY_5]) INTABLE[0xF7FE] &= 0xEF;

	if (key[KEY_Q]) INTABLE[0xFBFE] &= 0xFE;
	if (key[KEY_W]) INTABLE[0xFBFE] &= 0xFD;
	if (key[KEY_E]) INTABLE[0xFBFE] &= 0xFB;
	if (key[KEY_R]) INTABLE[0xFBFE] &= 0xF7;
	if (key[KEY_T]) INTABLE[0xFBFE] &= 0xEF;

	if (key[KEY_A]) INTABLE[0xFDFE] &= 0xFE;
	if (key[KEY_S]) INTABLE[0xFDFE] &= 0xFD;
	if (key[KEY_D]) INTABLE[0xFDFE] &= 0xFB;
	if (key[KEY_F]) INTABLE[0xFDFE] &= 0xF7;
	if (key[KEY_G]) INTABLE[0xFDFE] &= 0xEF;

	if (key[KEY_RSHIFT]) INTABLE[0x7FFE] &= 0xFD; /* SYMBOL SHIFT */
	if (key[KEY_LSHIFT]) INTABLE[0xFEFE] &= 0xFE; /* CAPS SHIFT */

	if (key[KEY_Z]) INTABLE[0xFEFE] &= 0xFD;
	if (key[KEY_X]) INTABLE[0xFEFE] &= 0xFB;
	if (key[KEY_C]) INTABLE[0xFEFE] &= 0xF7;
	if (key[KEY_V]) INTABLE[0xFEFE] &= 0xEF;

	if (key[KEY_0]) INTABLE[0xEFFE] &= 0xFE;
	if (key[KEY_9]) INTABLE[0xEFFE] &= 0xFD;
	if (key[KEY_8]) INTABLE[0xEFFE] &= 0xFB;
	if (key[KEY_7]) INTABLE[0xEFFE] &= 0xF7;
	if (key[KEY_6]) INTABLE[0xEFFE] &= 0xEF;

	if (key[KEY_P]) INTABLE[0xDFFE] &= 0xFE;
	if (key[KEY_O]) INTABLE[0xDFFE] &= 0xFD;
	if (key[KEY_I]) INTABLE[0xDFFE] &= 0xFB;
	if (key[KEY_U]) INTABLE[0xDFFE] &= 0xF7;
	if (key[KEY_Y]) INTABLE[0xDFFE] &= 0xEF;

	if (key[KEY_ENTER]) INTABLE[0xBFFE] &= 0xFE;
	if (key[KEY_L]) INTABLE[0xBFFE] &= 0xFD;
	if (key[KEY_K]) INTABLE[0xBFFE] &= 0xFB;
	if (key[KEY_J]) INTABLE[0xBFFE] &= 0xF7;
	if (key[KEY_H]) INTABLE[0xBFFE] &= 0xEF;

	if (key[KEY_TAB]) { INTABLE[0xFEFE] &= 0xFE;INTABLE[0x7FFE] &=0xFD; }

	if (key[KEY_SPACE]) INTABLE[0x7FFE] &= 0xFE;
	if (key[KEY_M]) INTABLE[0x7FFE] &= 0xFB;
	if (key[KEY_N]) INTABLE[0x7FFE] &= 0xF7;
	if (key[KEY_B]) INTABLE[0x7FFE] &= 0xEF;
	if (key[KEY_BACKSPACE]) { INTABLE[0xFEFE] &=0xFE; INTABLE[0xEFFE] &=0xFE; }
}
