...
%endif
   ...
   mov [_z80_halted], byte 0 ; Un-HALT the Z80
...
%macro DO_BIT 2
   ...
   test %1, (1 << %2)
  %ifdef EMULATE_BITS_3_5
   ; bit 3 and 5 are set from the register
   ; (actually they are bizarrely set, but
   ; there's no point emulating that, it's just
   ; plain silly...)
   %ifnidn %1, dl
    mov dl, %1    ; put in DL if not there already
   %endif
   mov [_z80_flag35], dl
  %endif
...
%%ldir_loop:
%ifdef EMULATE_WEIRD_STUFF
   cmp [ezPC], word 0b0edh       ; CG - check for self modifiying code!
   jne near %%ldir_modified
%endif
...
; Enable interrupts
%macro EI 0
   mov dl, FLAG_P      ; set the P/V bit, for use in "LD I,A" etc
   mov [_z80_IFF1], dl
   mov [_z80_IFF2], dl
...
really_finish:   
   ; merge bits 3 and 5 of the flags into the regs
   and zF, ~(FLAG_3|FLAG_5)
   mov dl, [_z80_flag35]
   and dl, FLAG_3|FLAG_5
   or zF, dl
...
%endif
   ...
   mov [_z80_halted], byte 0  ; Un-HALT us.
